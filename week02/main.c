...
typedef union {
    ...
    struct {
        ...
        uint8_t PWR_ON: 1;
        uint8_t ASSIST_LEVEL : 2;
        uint8_t LIGHT_BRIGHT : 4;
        uint16_t RESERVED : 9;
...
// HỌC VIÊN BẮT ĐẦU VIẾT CODE TỪ ĐÂY
void (*funct[])(void) = {drive_eco,drive_normal,drive_sport};
// HỌC VIÊN KẾT THÚC VIẾT CODE
...
int main() {
    ...
    // HỌC VIÊN BẮT ĐẦU VIẾT CODE TỪ ĐÂY
    if (my_bike.fields.ASSIST_LEVEL == 0){
        funct[0]();
    }
    else if (my_bike.fields.ASSIST_LEVEL == 1){
        funct[1]();
    }
    else if (my_bike.fields.ASSIST_LEVEL == 2){
        funct[2]();
    }
    else printf("Error");
    // HỌC VIÊN KẾT THÚC VIẾT CODE
